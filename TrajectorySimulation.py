...
class Trajectory():
    ...
    def TrajectoryPrediction (self):
        ...
        tfall = math.sqrt(m/(k*g))*math.log((2-math.exp(-k/m*Zmax)+2*math.sqrt(1-math.exp(-k/m*Zmax)))/math.exp(-k*Zmax/m))
        tfly = trise + tfall
        ...
        for t1 in n1:
            ...
            z_1 = -m/k*math.log(math.cos(math.atan(math.sqrt(k/(m*g))*Vz0))/(math.cos(math.atan(math.sqrt(k/(m*g))*Vz0)-t1*math.sqrt(g*k/m))))
            z1.append(z_1)
        ...
        for t2 in n2:
            ...
            z_2 = Zmax + m/k*math.log(4*math.exp(math.sqrt(k*g/m)*(t2-trise))/np.square((1+math.exp(math.sqrt(k*g/m)*(t2-trise)))))
            z2.append(z_2)
        ...
        height = m/(2*k)*np.square(math.log(1+math.exp(2*trise*math.sqrt(k*g/m))))/(4*math.exp(2*trise*math.sqrt(k*g/m)))
        peak = height * 1.0936
